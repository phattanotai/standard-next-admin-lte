...
export default class TopupMember extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    async handleSubmit() {
        try {
            if (this.state.agent_code !== "" && this.state.mem_name !== "" && this.state.mem_balance !== "" && this.state.deposit !== "" && this.state.note !== "") {
                ...
                const res = await ServiceMember.topup(a,this.state.mem_username);
                console.log('res : ' + res.data);
                if (res.data.status === 2000) {
                    console.log('topup Member Success!!!');
                    this.dialog.show({
                        title: 'Slot Admin information',
                        body: 'Top up a Member : ' + this.state.mem_username + ' is successfully.',
                        actions: [
                            Dialog.OKAction(() => {
                                Router.push("/member");
                            })
    ...
    render() {
        ...
        return <AdminLayoutHoc contentTitle={'Topup Member'} contentTitleButton={<i className="fa fa-2x fa-money" />} url={this.props.url}>
                        ...
                        <div className="card-body mt-3">
                            ...
                            <div className="form-group">
                                ...
                                <div className="col-sm-12">
                                    <div className="input-group mb-3 input-group-sm">
                                    <div className="input-group-prepend col-2">
                                            <span className="input-group-text col-12 bg-dark ">top up</span>
                                        </div>
                                        <input type="text" className="form-control " id="validation-ex3"
                                            //placeholder="Password"
