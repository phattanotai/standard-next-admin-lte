...
export default class Agents extends React.Component {
    ...
    componentDidMount() {
        ...
        ServiceUserAgent.getUserAgentFromAgentCode(Cookies.get("agentcode")).then((res) => {
            console.log(res.data);
            const { data, status } = res.data;
            if (status === 2000) {
                console.log("data length : " + data.length);
    ...
    render() {
        return <AdminLayoutHoc
            ...
            contentTitleButton={<Link href="/users/add">
                <button type="button" className="btn btn-outline-success btn-sm"><i className="fa fa-user-circle fa-fw" /> Add a new user agent</button>
            </Link>}
            url={this.props.url}
        >
            <div className="row">
