...
export default class Index extends React.Component {
    ...
    componentDidMount() {
        ...
        ServiceMemberShare.getShareAgent(Cookies.get("agentcode")).then((res) => {
            console.log(res.data);
            const { data, status } = res.data;
            if (status === 2000) {
                console.log("data length : " + data.length);
                                                ...
                                                if (name != "") {
                                                    this.setState({
                                                        data: this.state.rawdata.filter((data) => {
                                                            ...
                                                            if (data.share_redeem.indexOf(name) !== -1) {
                                                                return data.share_redeem.indexOf(name) !== -1;
                                                            }
                                                        })
                                                    });
                                ...
                                <tbody>
                                    {this.state.data.map((bonus, index) => {
                                        ...
                                        if (index >= this.state.startrow && index < this.state.endrow)
                                            ...
                                            return (
                                                <tr key={index}>
                                                    <td className="py-1" >{bonus.share_redeem}</td>
                                                    <td className="py-1" >{bonus.share_member}</td>                                                   
                                                    <td className="py-1" style={{ textAlign: "center" }}>
                                                        <button
                                                            type="button"
                                                            className= {"btn btn-primary btn-icon-text btn-sm"}
                                                            onClick={() => {
                                                                console.log("edit redeem id " + bonus.share_member);
                                                                Cookies.set("redeemId", bonus.share_member);
                                                                Router.push("/member_share/add");
                                                            }}
                                                            title='Redeem'
