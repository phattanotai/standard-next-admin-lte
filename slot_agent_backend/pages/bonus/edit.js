...
export default class EditNews extends React.Component {
    ...
    async EditBonus() {
        ...
        const a = {
            bonus_name: this.state.bonus_name, bonus_type: bonus_type,
            bonus_img: this.state.bonus_img, bonus_status: bonus_status
        };
        console.log('Edit Bonus start...');
        const res2 = await ServiceBonus.editBonus(a,this.state.bonusid);
        console.log('res : ' + res2.data);
        if (res2.data.status === 2000) {
            console.log('Edit Bonus Success!!!');
            this.dialog.show({
    ...
    componentDidMount() {
        ...
        console.log('Bonus ID : ' + this.state.bonusid);

        ServiceBonus.getBonusFromId(this.state.bonusid).then((res) => {
            ...
            if (status === 2000) {
                ...
                const { bonus_name, bonus_img, bonus_type, bonus_status } = data[0];
                if (bonus_img !== '') {
                    this.setState({ imagePreviewUrl: ServiceImage.imageshow(bonus_img),bonus_img: bonus_img });
                } else {
                    this.setState({ imagePreviewUrl: ServiceImage.imageshow("1594692624651-no-image-available.png") });
                }
                let b_type = 'โปรโมชั่น';
                if (bonus_type === '2') {
                    //b_type = 'ภาระกิจมือใหม่';
                    this.setState({ bonus_type_text: "ภาระกิจมือใหม่" });
                } else {
                    this.setState({ bonus_type_text: "โปรโมชั่น" });
                }
                this.setState({ bonus_name: bonus_name, switch1: Boolean(Number(bonus_status))});
            } else {
    ...
    render() {
        ...
        return <AdminLayoutHoc contentTitle={'Edit Bonus'} contentTitleButton={<i className="fa fa-2x fa-tachometer" />} url={this.props.url}>
