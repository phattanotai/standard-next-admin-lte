...
import { ServiceBonus } from "../../service";
...
export default class index extends React.Component {
        ...
        if (name != "") {
            this.setState({
                data: this.state.rawdata.filter((bonus) => {
                    return bonus.bonus_name.indexOf(name) !== -1;
                })
            });
    ...
    componentDidMount() {
        ...
        ServiceBonus.listBonus(Cookies.get("agentcode")).then((res) => {
            console.log(res.data);
            const { data, status } = res.data;
            if (status === 2000) {
                console.log("data length : " + data.length);
    ...
    render() {
        return <AdminLayoutHoc
            contentTitle={'Bonus'}
            contentTitleButton={<Link href="/bonus/add">
                <button type="button" className="btn btn-outline-success btn-sm"><i className=" fa fa-tachometer" /> Add a new Bonus</button>
            </Link>}
            url={this.props.url}
        >
            <div className="row">
                        ...
                        <div className="card-body table-responsive p-0">
                            <table className="table table-hover table-striped table-bordered">
                                <thead  class="table-dark">
                                    <tr >
                                        <th style={{ width: '60%' }}>Bonus Name</th>
                                ...
                                <tbody>
                                    {this.state.data.map((bonus, index) => {
                                        ...
                                        if (index >= this.state.startrow && index < this.state.endrow)
                                            ...
                                            return (
                                                <tr key={index}>
                                                    <td className="py-1" ><label className={`badge badge-${classBadge}`}>{mystatus}</label>{' ' + bonus.bonus_name}</td>
                                                    <td className="py-1" >{b_type}</td>
                                                    <td className="py-1">
                                                        <img
                                                            src={bonus.bonus_img ? ServiceImage.imageshow(bonus.bonus_img) : ServiceImage.imageshow("1594692624651-no-image-available.png")}
                                                            style={{
                                                                flex: 1,
                                                                height: "50px",
                                                                width: null,
