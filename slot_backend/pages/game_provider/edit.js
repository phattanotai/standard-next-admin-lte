...
import { ServiceBrand, ServiceGameProvider } from "../../service";
...
export default class EditGame extends React.Component {
    ...
    async editGame() {
        ...
        const a = {
            game_code: this.state.game_code, game_name: this.state.game_name, game_img: this.state.game_img, game_new: game_new, 
            game_status: game_status, original_game_img: this.state.original_game_img,
            game_img_app: this.state.game_img_app, original_game_img_app: this.state.original_game_img_app
        };
        console.log('Edit Game Provider start...');
        const res2 = await ServiceGameProvider.editGames(a, this.state.gameid);
    ...
    componentDidMount() {
        ...
        ServiceGameProvider.getGames(this.state.gameid).then((res) => {
            console.log(res.data);
            const { data, status } = res.data;
            if (status === 2000) {
                console.log("data length : " + data.length);
                const { game_code, game_name, game_brand, game_img,
                   game_new, game_status, game_img_app } = data[0];
                if (game_img !== '') {
                ...
                this.setState({
                    game_code: game_code, game_name: game_name, game_brand: game_brand, original_game_img: game_img,
                    switch2: Boolean(Number(gamenew)), switch1: Boolean(Number(game_status)),
                    game_img: game_img, game_img_app: game_img_app
                });
            } else {
    ...
    render() {
        ...
        return <AdminLayoutHoc contentTitle={'Edit Game Provider'} contentTitleButton={<i className="fa fa-2x fa-cubes" />} url={this.props.url}>
