...
import { ServiceMember } from "../../service";
import { ServiceWallets } from "../../service";
...
export default class Wallets extends React.Component {
    ...
    async deletewallet(id) {
        ...
        try {
            const res = await ServiceWallets.deleteWallets(id);
    ...
    componentDidMount() {
        ...
        ServiceMember.listMember().then((res) => {
            ...
            if (status === 2000) {
                ...
                ServiceWallets.getWalletsFromAgentCode(Cookies.get("agentcode")).then((res) => {
                    console.log(res.data);
                    const { data, status } = res.data;
                    if (status === 2000) {
                        console.log("data length : " + data.length);
    ...
    render() {
        return <AdminLayoutHoc
            ...
            contentTitleButton={<Link href="/wallets/add">
                <button type="button" className="btn btn-outline-success btn-sm"><i className="fa fa-money fa-fw" /> Add a new wallet</button>
            </Link>}
            url={this.props.url}
        >
            <div className="row">
                                                    ...
                                                    if (name != "") {
                                                        this.setState({
                                                            data: this.state.rawdata.filter((data) => {
                                                                ...
                                                                } else if (data.brand_code.indexOf(name) !== -1) {
                                                                    return data.brand_code.indexOf(name) !== -1;
                                                                } else if (data.game_username.indexOf(name) !== -1) {
                                                                    return data.game_username.indexOf(name) !== -1;
                                                                }
                            ...
                            <table className="table table-hover table-striped table-bordered">
                                <thead class="table-dark">
                                    <tr>
                                        ...
                                        <th width={'20%'}>mem username</th>
                                        <th width={'20%'}>brand code</th>
                                        <th width={'20%'}>game username</th>
                                        <th width={'20%'}>game password</th>

                                        <th style={{ textAlign: "center" }}>Command</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {this.state.data.map((wallet, index) => {
                                                    ...
                                                    <td className="py-1" style={{ textAlign: "center" }}>
                                                        <button
                                                            ...
                                                            onClick={() => {
                                                                ...
                                                                Cookies.set("walletid", wallet._id);
