...
export default class AddAgent extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            agent_website: "",
            agent_master: "",
            agent_type: "Master", // Master or Agent
            agent_percent: "",
            agent_percent_aff: "",
            agent_percent_aff2: "",
            agent_percent_aff3: "",
    ...
    async handleSubmit() {
        try {
            if (this.state.agent_code !== "" && this.state.agent_name) {
                ...
                /* if (this.state.agent_percent == "") {
                    this.setState({agent_percent: 0});
                }
                if (this.state.agent_percent_aff == "") {
                    this.setState({agent_percent_aff: 0});
                }
                if (this.state.agent_percent_aff2 == "") {
                    this.setState({agent_percent_aff2: 0});
                }
                if (this.state.agent_percent_aff3 == "") {
                    this.setState({agent_percent_aff3: 0});
                }
                if (this.state.agent_percent_aff4 == "") {
                    this.setState({agent_percent_aff4: 0});
                }
                if (this.state.agent_percent_aff5 == "") {
                    this.setState({agent_percent_aff5: 0});
                }
                if (this.state.agent_comm == "") {
                    this.setState({agent_comm: 0});
                } */
                const a = {
                ...
                const res = await ServiceAgent.createAgent(a);
                console.log('res : ' + res.data);
                if (res.data.status === 2000) {
                    console.log('Add Agent Success!!!');
                    this.dialog.show({
                        title: 'Slot Admin information',
                        body: 'Add a agent : ' + a.agent_code + ' to system is successfully.',
                        actions: [
                            Dialog.OKAction(() => {
