...
export default class News extends React.Component {
    ...
    componentDidMount() {
        ...
        ServiceNews.listNews(Cookies.get("agentcode")).then((res) => {
            console.log(res.data);
            const { data, status } = res.data;
            if (status === 2000) {
                console.log("data length : " + data.length);
    ...
    render() {
        return <AdminLayoutHoc
            contentTitle={'News'}
            contentTitleButton={<Link href="/news/add">
                <button type="button" className="btn btn-outline-success btn-sm"><i className="fa fa-newspaper-o" /> Add a new News</button>
            </Link>}
            url={this.props.url}
        >
            <div className="row">
                                                ...
                                                if (name != "") {
                                                    this.setState({
                                                        data: this.state.rawdata.filter((data) => {
                                                            ...
                                                            if (data.news_text.indexOf(name) !== -1) {
                                                                return data.news_text.indexOf(name) !== -1;
                                                            }
                                                        })
                                                    });
                                ...
                                <tbody>
                                    {this.state.data.map((news, index) => {
                                        ...
                                        if (index >= this.state.startrow && index < this.state.endrow)
                                            ...
                                            return (
                                                <tr key={index}>
                                                    <td className="py-1" ><label className={`badge badge-${classBadge}`}>{mystatus}</label>{' ' + news.news_text}</td>
                                                    ...
                                                    <td className="py-1" style={{ textAlign: "center" }}>
                                                        <button
                                                            ...
                                                            onClick={() => {
                                                                console.log("edit mews id " + news._id);
                                                                Cookies.set("newsid", news._id);
