...
import { ServiceShare } from "../../service";
...
export default class index extends React.Component {
    ...
    componentDidMount() {
        ...
        ServiceShare.getShareAgent(Cookies.get("agentcode")).then((res) => {
            console.log(res.data);
            const { data, status } = res.data;
            if (status === 2000) {
                console.log("data length : " + data.length);
                    ...
                    <div className="card">
                        ...
                        <div className="card-body table-responsive p-0">
                            <table className="table table-hover table-striped table-bordered">
                                ...
                                <tbody>
                                    {this.state.data.map((share, index) => {                                        
                                        if (index >= this.state.startrow && index < this.state.endrow)
                                            //console.log("userid" + user.id);
                                            return (
                                                <tr key={index}>                                                    
                                                    <td className="py-1" >{share.share_message}</td>
                                                    <td className="py-1">
                                                        <img
                                                            src={share.share_img ? ServiceImage.imageshow(share.share_img) : ServiceImage.imageshow("1594692624651-no-image-available.png")}
                                                            style={{
                                                                flex: 1,
                                                                height: "50px",
                                                                width: null,
                                                    ...
                                                    <td className="py-1" style={{ textAlign: "center" }}>
                                                        <button
                                                            ...
                                                            onClick={() => {
                                                                console.log("edit share id " + share._id);
                                                                Cookies.set("shareid", share._id);
