...
export default class Member extends React.Component {
        ...
        if (name != "") {
            this.setState({
                data: this.state.rawdata.filter((data) => {
                    ...
                    } else if (data.mem_tel.indexOf(name) !== -1) {
                        return data.mem_tel.indexOf(name) !== -1;
                    } else if (data.mem_line.indexOf(name) !== -1) {
                        return data.mem_line.indexOf(name) !== -1;
    ...
    componentDidMount() {
        ...
        ServiceMember.getMemberAgent(Cookies.get("agentcode")).then((res) => {
            console.log(res.data);
            const { data, status } = res.data;
            if (status === 2000) {
                console.log("data length : " + data.length);
                ...
                <div className="col-12">
                    <div className="card">
                        ...
                        <div className="card-body table-responsive p-0">
                            <table className="table table-hover table-striped table-bordered table-sm" cellspacing="0" id="table">
                                <thead class="table-dark">
                                    <tr >
                                        <th width={'5%'}>Status</th>
                                        <th width={'18%'} class="sortable" onClick={() => {
                                            this.setState({
                                        ...
                                        <th width={'15%'} class="sortable" onClick={() => {
                                            this.setState({
                                                data: this.state.rawdata.sort((x, y) => {
                                        ...
                                        <th width={'15%'} class="sortable" onClick={() => {
                                            this.setState({
                                                data: this.state.rawdata.sort((x, y) => {
                                                    let a = x.mem_tel.toUpperCase(),
                                                        b = y.mem_tel.toUpperCase();
                                        ...
                                        <th width={'10%'} class="sortable" onClick={() => {
                                            this.setState({
                                                data: this.state.rawdata.sort((x, y) => {
                                                    let a = x.mem_line.toUpperCase(),
                                                        b = y.mem_line.toUpperCase();
                                            ...
                                            this.setState({
                                                data: this.state.rawdata.sort((x, y) => {
                                                    if (this.state.fsort5) {
                                                        return x.mem_balance - y.mem_balance;
                                                    } else {
                                                        return y.mem_balance - x.mem_balance;
                                                    }
                                        ...
                                        {/* <th>RegType</th> */}
                                        <th style={{ textAlign: "center" }} width={'20%'} >Command</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {this.state.data.map((member, index) => {
                                        ...
                                        if (index >= this.state.startrow && index < this.state.endrow)
                                            ...
                                            return (
                                                <tr key={index}>
                                                    <td className="py-1"><label className={`badge badge-${classBadge}`} width='50%'>{mystatus}</label></td>
                                                    <td className="py-1">{member.mem_username}</td>
                                                    <td className="py-1">{member.mem_password}</td>
                                                    <td className="py-1">{member.mem_name}</td>
                                                    <td className="py-1">{member.mem_tel}</td>
                                                    <td className="py-1">{member.mem_line}</td>
                                                    <td className="py-1">{member.mem_balance}</td>
                                                    {/* <td className="py-1">{member.register_type}</td> */}
                                                    <td className="py-1" style={{ textAlign: "center" }}>
                                                        <button
                                                            type="button"
                                                            className="btn btn-warning"
                                                            onClick={() => {
                                                                console.log("edit member id " + member._id);
                                                        ...
                                                        <button
                                                            ...
                                                            onClick={() => {
                                                                console.log("deposit member id " + member._id);
                                                                Cookies.set("memberid", member._id);
                                                                //Cookies.set("uagentid", uagent.agent_code);
                                                                Router.push("/member/deposit");
                                                            }}
                                                            title='Deposit'
                                                        ...
                                                        <button
                                                            ...
                                                            onClick={() => {
                                                                console.log("withdraw member id " + member._id);
                                                                Cookies.set("memberid", member._id);
                                                                //Cookies.set("uagentid", uagent.agent_code);
