...
import { ServiceMemberShare } from "../../service";
...
export default class AddMember extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            mem_line_up:  Cookies.get("redeemId"),
    ...
    async handleSubmit() {
        try {
            if (this.state.agent_code !== "" && this.state.mem_name !== "" && this.state.mem_balance !== "" ) {
                    ...
                    const a = {
                        agent_code: this.state.agent_code,mem_date_add: "", mem_username: "",mem_password: "",mem_name: this.state.mem_name,
                        mem_tel: this.state.mem_tel, mem_line: this.state.mem_line, mem_balance: this.state.mem_balance,
                        mem_status: mem_status,external_id: "",last_login: "",register_type: this.state.register_type,mem_line_up: this.state.mem_line_up
                    };
                    const res = await ServiceMember.createMember(a);
                    console.log('res : ' + res.data);
                    if (res.data.status === 2000) {
                        console.log('Add Member Success!!!');
                        this.dialog.show({
                            title: 'Slot Admin information',
                            body: 'Add a Member : ' + a.mem_name + ' to system is successfully.',
                            actions: [
                                Dialog.OKAction(() => {
                                    Router.push("/member");
                                })
    ...
    componentDidMount() {
        ...
        /* ServiceMemberShare.getShare(this.state.redeemId).then((res) => {
            ...
            if (status === 2000) {
                ...
                const { mem_line_up } = data[0];
                this.setState({
                    mem_line_up: mem_line_up
                });
                                ...
                                <div className="col-sm-12">
                                    <div className="input-group mb-3 input-group-sm">
                                        <div className="input-group-prepend col-2">
                                            <span className="input-group-text bg-dark col-12">member line up</span>
                                        </div>
                                        <input type="text" className="form-control " id="validation-ex2"
                                            //placeholder="agent lineup"
                                            onChange={(e) => this.setState({ mem_line_up: e.target.value })}
                                            value={this.state.mem_line_up}
                                            disabled={true}
                                            required
                                        />
                                    </div>
